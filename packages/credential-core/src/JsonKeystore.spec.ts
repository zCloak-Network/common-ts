import type { KeyringPair$Json } from '@polkadot/keyring/types';

import { jest } from '@jest/globals';
import { stringToU8a, u8aEq } from '@polkadot/util';

import { JsonKeystore } from './JsonKeystore';

describe('JsonKeystore', (): void => {
  beforeEach(() => {
    jest.setTimeout(30000);
    process.env.NODE_ENV = 'test';
  });

  it('encrypt and decrypted', async () => {
    const jsonA1 = JSON.parse(
      '{"encoded":"","encoding":{"content":["pkcs8","sr25519"],"type":["scrypt","xsalsa20-poly1305"],"version":"3"},"address":"4r8yoQsPneNCRMzn79WwDaNR69cRMxM2MQQ346quzqwNnBD7","meta":{"genesisHash":"0xa0c6e3bac382b316a68bca7141af1fba507207594c761076847ce358aeedcc21","isHardware":false,"name":"a","tags":[],"whenCreated":1655811964724}}'
    ) as KeyringPair$Json;
    const jsonA2 = JSON.parse(
      '{"encoded":"","encoding":{"content":["pkcs8","ed25519"],"type":["scrypt","xsalsa20-poly1305"],"version":"3"},"address":"4rgxQpHuk5c4LwQSWKKejrA3A6QfCCWkDLYas7YJtXn5YSda","meta":{"genesisHash":"0xa0c6e3bac382b316a68bca7141af1fba507207594c761076847ce358aeedcc21","isHardware":false,"name":"A1","tags":[],"whenCreated":1655813061290}}'
    ) as KeyringPair$Json;
    const jsonB1 = JSON.parse(
      '{"encoded":"","encoding":{"content":["pkcs8","sr25519"],"type":["scrypt","xsalsa20-poly1305"],"version":"3"},"address":"4t5WoEr1Cc6RDr829RvsVea1svQ6GfvrotrJWYaabuK7tr33","meta":{"genesisHash":"0xa0c6e3bac382b316a68bca7141af1fba507207594c761076847ce358aeedcc21","isHardware":false,"name":"c","tags":[],"whenCreated":1655811993553}}'
    ) as KeyringPair$Json;
    const jsonB2 = JSON.parse(
      '{"encoded":"","encoding":{"content":["pkcs8","ed25519"],"type":["scrypt","xsalsa20-poly1305"],"version":"3"},"address":"4q5PgwPdg8owKhbLiAZJ26zhEA8BpqPZezWqyeKhwtKYHJCM","meta":{"genesisHash":"0xa0c6e3bac382b316a68bca7141af1fba507207594c761076847ce358aeedcc21","isHardware":false,"name":"C1","tags":[],"whenCreated":1655813084542}}'
    ) as KeyringPair$Json;

    const jsonKeystoreA = new JsonKeystore(jsonA1, jsonA2);
    const jsonKeystoreB = new JsonKeystore(jsonB1, jsonB2);
    const encrypPairA = jsonKeystoreA.encryptPair;
    const encrypPairB = jsonKeystoreB.encryptPair;

    encrypPairA.unlock('1');
    encrypPairB.unlock('1');

    const encrypted = await jsonKeystoreA.encrypt({
      alg: 'x25519-xsalsa20-poly1305',
      data: stringToU8a('bbb'),
      publicKey: encrypPairA.publicKey,
      peerPublicKey: encrypPairB.publicKey
    });

    const decrypted = await jsonKeystoreB.decrypt({
      ...encrypted,
      publicKey: encrypPairB.publicKey,
      peerPublicKey: encrypPairA.publicKey
    });

    expect(u8aEq(decrypted.data, stringToU8a('bbb')));
  });
});
